...
INNER JOIN dim.Events AS e ON g.EventID = e.EventID
...
LEFT JOIN (
	...
	INNER JOIN stat.MoveScores AS ms ON m.GameID = ms.GameID
		...
		AND m.MoveNumber = ms.MoveNumber
		AND ms.ScoreID = 1
	INNER JOIN lake.Games g ON m.GameID = g.GameID
	INNER JOIN dim.Sources s ON g.SourceID = s.SourceID
	INNER JOIN dim.Events e ON g.EventID = e.EventID
	INNER JOIN dim.Colors c ON m.ColorID = c.ColorID
	INNER JOIN dim.Players p ON g.BlackPlayerID = p.PlayerID
...
WHERE s.SourceName = 'Personal'
AND (wp.SelfFlag = 1 OR bp.SelfFlag = 1)
