...
JOIN lake.Games AS g ON m.GameID = g.GameID
JOIN dim.Sources AS s ON g.SourceID = s.SourceID
JOIN dim.Players AS wp ON g.WhitePlayerID = wp.PlayerID
JOIN dim.Players AS bp ON g.BlackPlayerID = bp.PlayerID
JOIN dim.ECO AS e ON g.ECOID = e.ECOID
JOIN dim.Events AS ev ON g.SourceID = ev.SourceID
	AND g.EventID = ev.EventID
LEFT JOIN dim.TimeControlDetail AS td ON g.TimeControlDetailID = td.TimeControlDetailID
JOIN dim.TimeControls AS tc ON td.TimeControlID = tc.TimeControlID
LEFT JOIN dim.Sites AS st ON g.SiteID = st.SiteID
LEFT JOIN fact.Game AS wg ON g.GameID = wg.GameID
	AND wg.ColorID = 1
LEFT JOIN fact.Game AS bg ON g.GameID = bg.GameID
	AND bg.ColorID = 2
LEFT JOIN fact.GameScores AS gsw ON g.GameID = gsw.GameID
	AND gsw.ColorID = 1
	AND gsw.ScoreID = dbo.GetSettingValue('Default Score')
LEFT JOIN fact.GameScores AS gsb ON g.GameID = gsb.GameID
	AND gsb.ColorID = 2
