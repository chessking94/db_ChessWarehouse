...
LEFT JOIN (
	...
	GROUP BY
		wp.PlayerID
) AS me ON wp.PlayerID = me.PlayerID
LEFT JOIN (
	SELECT
		wp.PlayerID
		,COUNT(m.MoveNumber) AS MovesAnalyzed
	...
	GROUP BY
		wp.PlayerID
) opp ON wp.PlayerID = opp.PlayerID
...
LEFT JOIN (
	...
	GROUP BY
		bp.PlayerID
) AS me ON bp.PlayerID = me.PlayerID
LEFT JOIN (
	SELECT
		bp.PlayerID
		,COUNT(m.MoveNumber) AS MovesAnalyzed
	...
	GROUP BY
		bp.PlayerID
) AS opp ON bp.PlayerID = opp.PlayerID
...
) AS me ON (CASE WHEN wp.SelfFlag = 1 THEN bp.PlayerID ELSE wp.PlayerID END) = me.PlayerID
LEFT JOIN (
	SELECT
...
) opp ON (CASE WHEN wp.SelfFlag = 1 THEN bp.PlayerID ELSE wp.PlayerID END) = opp.PlayerID
