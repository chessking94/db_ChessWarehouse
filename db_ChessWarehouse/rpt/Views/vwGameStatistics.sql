...
FROM fact.Game AS a
INNER JOIN fact.Game AS b ON a.GameID = b.GameID
	AND a.ColorID <> b.ColorID
INNER JOIN lake.Games AS g ON a.GameID = g.GameID
INNER JOIN dim.Sources AS s ON g.SourceID = s.SourceID
INNER JOIN dim.Players AS p ON a.PlayerID = p.PlayerID
...
FROM fact.Game AS a
JOIN fact.Game AS b ON a.GameID = b.GameID
	AND a.ColorID <> b.ColorID
JOIN lake.Games AS g ON a.GameID = g.GameID
JOIN dim.Sources AS s ON g.SourceID = s.SourceID
JOIN dim.Players AS p ON a.PlayerID = p.PlayerID
...
INNER JOIN fact.Game AS b ON a.GameID = b.GameID
	AND a.ColorID <> b.ColorID
LEFT JOIN fact.GameScores AS gsa ON a.GameID = gsa.GameID
	AND a.ColorID = gsa.ColorID
	AND gsa.ScoreID = dbo.GetSettingValue('Default Score')
LEFT JOIN fact.GameScores AS gsb ON b.GameID = gsb.GameID
