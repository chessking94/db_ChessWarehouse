...
BEGIN
	...
	SELECT
		...
		,TimeControlID
		,ScoreID
		,COUNT(EventID) AS Ct_Score
		,AVG(Score) AS Avg_Score
		,STDEV(Score) AS SD_Score
	...
	INNER JOIN #tmpeventscores AS t ON es.SourceID = t.SourceID
		AND es.TimeControlID = t.TimeControlID
		AND es.ScoreID = t.ScoreID
	...
	FROM fact.EventScores AS es
	INNER JOIN #tmpeventscores AS t ON t.SourceID = 3
	...
	*/
	...
	FROM fact.EventScores AS es
	INNER JOIN fact.Event AS fe ON es.EventID = fe.EventID
